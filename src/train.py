...
def define_access_gap_target(df):
    ...
    print(f" Dropped {initial_rows - len(df)} rows with missing density for target definition.")

    # Bottom 20% of valid areas
    threshold = df[density_col].quantile(0.20)
    print(f"Density threshold (20th percentile): {threshold:.2f} providers per 10k")
